...
# Load full Fashion-MNIST dataset
def load_full_fashion_mnist(train=True):
    return datasets.FashionMNIST(root='./data_fashion', train=train, download=True)
...
def create_label_skew_indices(client_id, total_samples=60000, alpha_label=0.7, alpha_quantity=0.5, seed=42):
    ...
    quantity_proportions = np.random.dirichlet([alpha_quantity] * 5)
    ...
    client_total = np.sum(class_allocations)
    class_allocations[-1] = max(1, client_total - np.sum(class_allocations[:-1]))

    for class_idx in range(num_classes):
        class_indices = np.where(all_targets == class_idx)[0]
        np.random.shuffle(class_indices)
        needed = class_allocations[class_idx]
        
        if needed > 0:
            selected = class_indices[:needed]
...
class ClientDataset(torch.utils.data.Dataset):
    def __init__(self, indices, transform):
        self.full_dataset = datasets.FashionMNIST(root='./data_fashion', train=True, download=True)
...
def build_hybrid_loader(client_id, scenario):
    ...
    test_dataset = datasets.FashionMNIST(root='./data_fashion', train=False, transform=test_transform, download=True)
...
def send_model_parameters(stub, model, accuracy, avg_loss, client_id, scenario, order_id, train_loader, round_num):
    ...
    metadata = fl_pb2.ModelMetadata(
        ...
        data_quality_score=len(train_loader.dataset)/50000,
